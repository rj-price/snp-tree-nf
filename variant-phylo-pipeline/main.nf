...
workflow {
    ...
    //
    // MODULE: Variant calling with BCFtools mpileup and call
    //
    BCFTOOLS_MPILEUP(
    ...
    //
    VCF_TO_PHYLIP(
        BCFTOOLS_FILTER.out.vcf.map { _meta, vcf -> vcf }.collect()
    )
    ...
    //
    // MODULE: Convert VCF to NEXUS format (optional, separate output)
    //
    VCF_TO_NEXUS(
        BCFTOOLS_FILTER.out.vcf.map { _meta, vcf -> vcf }.collect()
    )
